...
const clearTerminal = require('./util/clear-terminal');
...
async function init(siteName, options) {
    ...
    if (options.force) {
      ...
      if (current) {
        ...
        if (useCurrentPath) {
          ...
          console.log(existingProjectDirs);
          
          if (existingProjectDirs.length > 0) {
            const { overwrite } = await inquirer.prompt([{
              type: 'list',
              name: 'overwrite',
              message: `A project already exists in directory ${chalk.greenBright(targetPath)}. Overwrite?`,
              choices: [
            ...
            if (overwrite) {
              existingProjectDirs.forEach( (dir) => { fse.removeSync(dir) });
            } else {
              return;
            }
          }
        ...
        const { overwrite } = await inquirer.prompt([{
          type: 'list',
          name: 'overwrite',
          message: `Directory ${chalk.greenBright(targetPath)} already exists. Overwrite?`,
          choices: [
            {name: ' Overwrite', value: true},
            {name: ' Cancel', value: false},
          ],
  ...
  fse.writeFileSync(path.join(targetPath, 'vdg.config.json'), JSON.stringify(vandegraafConfig, null, 2));
