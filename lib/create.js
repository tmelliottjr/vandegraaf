...
async function create() {
  ...
  templates.forEach(async templateFile => {
    ...
    let pageContent = parsePageContent(ext, body, context);
    ...
    if (doLayout) {
      ...
      // layouts should be .hbs
      pageContent = parsePageContent(".hbs", layoutContent, context);
    }
...
function parsePageContent(ext, body, context) {
  switch (ext) {
    case ".md":
      return marked(body);
      break;
